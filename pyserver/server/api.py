    ...
    def _report_exception(*args, **kwargs):
        ...
        try:
            ...
            current_app.logger.error(
                "Exception during request %s: %.10000s" % (request, exception)
            )
...
def run_remote_procedure():
    ...
    try:
        ...
        return jsonify({"result": result, "jsonrpc": "2.0"})
    except Exception as e:
        print(traceback.format_exc())
        return jsonify({"error": {"code": -1, "message": str(e)}, "jsonrpc": "2.0"})
...
def send_downloadable_file():
    ...
    try:
        ...
        response = helpers.send_from_directory(
            dirname, basename, as_attachment=True, attachment_filename=basename
        )
        ...
        response.headers["data"] = json.dumps(
            {"error": {"code": -1, "message": str(e)}, "jsonrpc": "2.0"}
        )
